...
async def get_location_online(ip: str, user_agent: str) -> dict | None:
    """
    Get IP location online, availability not guaranteed, accuracy is relatively high
...
def get_location_offline(ip: str) -> dict | None:
    """
    Get IP location offline, accuracy not guaranteed, 100% available
...
async def parse_ip_info(request: Request) -> IpInfo:
    """
    Parse IP information from the request
...
def parse_user_agent_info(request: Request) -> UserAgentInfo:
    """
    Parse user agent information from the request
