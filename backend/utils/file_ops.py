...
def upload_file_verify(file: UploadFile) -> None:
    ...
    if file_ext == FileType.image:
        ...
        if file.size > settings.UPLOAD_IMAGE_SIZE_MAX:
            raise errors.RequestError(msg='Image exceeds maximum limit, please reselect')
        ...
        if file.size > settings.UPLOAD_VIDEO_SIZE_MAX:
            raise errors.RequestError(msg='Video exceeds maximum limit, please reselect')
...
async def upload_file(file: UploadFile) -> str:
    ...
    try:
        ...
        log.error(f'Failed to upload file {filename}: {str(e)}')
...
async def install_zip_plugin(file: UploadFile | str) -> str:
    ...
    if not zipfile.is_zipfile(file_bytes):
        raise errors.RequestError(msg='Invalid plugin zip file format')
    with zipfile.ZipFile(file_bytes) as zf:
        # Validate zip file
        plugin_namelist = zf.namelist()
        plugin_dir_name = plugin_namelist[0].split('/')[0]
        if not plugin_namelist:
            raise errors.RequestError(msg='Invalid plugin zip file content')
        if (
            ...
            raise errors.RequestError(msg='Required files missing in plugin zip')
...
async def parse_sql_script(filepath: str) -> list[str]:
    ...
    for statement in statements:
        if not any(statement.lower().startswith(_) for _ in ['select', 'insert']):
            raise errors.RequestError(msg='Illegal operation in SQL script file, only SELECT and INSERT are allowed')
