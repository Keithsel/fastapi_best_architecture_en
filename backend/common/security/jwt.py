...
async def create_access_token(user_id: int, multi_login: bool, **kwargs) -> AccessToken:
    ...
    :param multi_login: Allow multi-device login
    :param kwargs: Extra token info
    :return:
    """
    ...
    # Store token extra info separately
    if kwargs:
        await redis_client.setex(
...
async def create_new_token(
    ...
    if not redis_refresh_token or redis_refresh_token != refresh_token:
        raise errors.TokenError(msg='Refresh Token expired, please log in again')
...
async def get_current_user(db: AsyncSession, pk: int) -> User:
    ...
    if not user:
        raise errors.TokenError(msg='Invalid token')
    if not user.status:
        raise errors.AuthorizationError(msg='User is locked, please contact system administrator')
    if user.dept_id:
        if not user.dept.status:
            raise errors.AuthorizationError(msg='User department is locked, please contact system administrator')
    ...
    if user.roles:
        ...
        if all(status == 0 for status in role_status):
            raise errors.AuthorizationError(msg='User roles are all locked, please contact system administrator')
    return user
...
async def jwt_authentication(token: str) -> GetUserInfoWithRelationDetail:
    ...
    if not cache_user:
        ...
        # TODO: At the right time, replace with model_validate_json
